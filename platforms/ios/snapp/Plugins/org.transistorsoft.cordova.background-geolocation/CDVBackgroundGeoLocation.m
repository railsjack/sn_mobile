...
#define LOCATION_UPDATE_INTERVAL      10.0
...
- (void) stop:(CDVInvokedUrlCommand*)command
{
    ...
    [self stopUpdatingLocationForcfully];
...
//    }
   ...
   // NSLog(@"didUpdateLocations::> lat: %f ,  lng: %f", location.coordinate.latitude, location.coordinate.longitude);
...
- (void)getLocationCode:(id)sender {
       ...
       timer = [NSTimer scheduledTimerWithTimeInterval:LOCATION_UPDATE_INTERVAL target:self selector:@selector(getLocationMethod) userInfo:Nil repeats:YES];
}
...
-(void)getLocationMethod{
   ...
   // if ([self locationAge:location] > 5.0) return;
    ...
    NSMutableDictionary *returnInfo = [self locationToHash:location];
    [returnInfo setObject:@"current" forKey:@"location_type"];
    [self sync:returnInfo];
    
    
}
-(void)stopUpdatingLocationForcfully{
    [timer invalidate];
    timer = nil;
